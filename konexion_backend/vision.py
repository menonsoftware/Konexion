...
class VisionProcessor:
    ...
    def validate_images(self, images: List[Dict[str, Any]]) -> Tuple[bool, str]:
        ...
        for i, image in enumerate(images):
            ...
            if not data_url.startswith("data:"):
                return False, f"Image {i+1} data must be a valid data URL starting with 'data:'"
        ...
        for image in images:
            if image.get("data"):
                ...
                if base64_data:
                    ...
                    logger.debug(f"Added image to Ollama vision data: {image.get('name', 'Unknown')}")
        
        return text_content, images_list
    
    def format_vision_message(self, role: str, user_message: str, images: List[Dict[str, Any]], 
                            model_format: str = "groq") -> Dict[str, Any]:
        """
    ...
    def process_vision_request(self, model_name: str, user_message: str, 
        ...
        try:
            ...
            if images and not supports_vision:
                ...
                logger.warning(f"Model {model_name} does not support vision. User attempted to send {len(images)} images.")
                return result
...
def prepare_vision_message(role: str, user_message: str, images: List[Dict[str, Any]], 
                         model_format: str = "groq") -> Dict[str, Any]:
    """Format a message for vision models."""
    return vision_processor.format_vision_message(role, user_message, images, model_format)

def process_vision_request(model_name: str, user_message: str, 
                         images: List[Dict[str, Any]]) -> Dict[str, Any]:
